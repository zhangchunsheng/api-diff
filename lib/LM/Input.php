...
class Input {
    const LUOMOR_URL_ONLY = 0x01;
    const LUOMOR_URL_HOST_IGNORE = 0x02;

    const LUOMOR_URL_DEFAULT = 0x03; //\LM\Input::LUOMOR_URL_ONLY | \LM\Input::LUOMOR_URL_HOST_IGNORE;

    //we should filter all the not safe chars from the url
    private static $URL_NOT_SAFE_CHARS = array('<', '>', '"', "'");

    private static $validDomainRegex = array(
        self::LUOMOR_URL_ONLY => "/(\.luomor|^luomor|\.luomor-inc)\.(com|cn|org|name)$/"
    );

    static public function url($name, $flag = \LM\Input::LUOMOR_URL_DEFAULT){
    ...
    static public function filterUrl($url, $flag = \LM\Input::LUOMOR_URL_DEFAULT) {
        return self::isValidUrl($url, $flag) ? 
            str_replace(self::$URL_NOT_SAFE_CHARS, "", filter_var($url, FILTER_SANITIZE_URL)) : null;
    }

    static public function isValidUrl($url, $flag = \LM\Input::LUOMOR_URL_DEFAULT) {
        $urlParts = parse_url($url);
        if(isset($urlParts['host']) && preg_match(\LM\Input::$validDomainRegex[\LM\Input::LUOMOR_URL_ONLY], $urlParts['host'])) {
            return true;
        } else if(!isset($urlParts['host']) && ($flag & \LM\Input::LUOMOR_URL_HOST_IGNORE)){
            return true; 
        }
        return false;
    }
